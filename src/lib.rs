...
pub fn extract(openapi: &openapiv3::OpenAPI) -> Result<spec::Spec> {
    let components = openapi
        .components
        .as_ref()
        .ok_or_else(|| Error::InvalidSpec("missing components".into()))?;
    let property_schema = components
        .schemas
        .get("_types.mapping.Property")
        .ok_or_else(|| Error::InvalidSpec("missing _types.mapping.Property schema".into()))?
        .as_item()
        .ok_or_else(|| Error::InvalidSpec("_types.mapping.Property is not an item".into()))?;
    let discriminator = &property_schema
        .schema_data
        .discriminator
        .as_ref()
        .ok_or_else(|| Error::InvalidSpec("missing discriminator".into()))?
        .mapping;
    let mut properties: HashMap<String, spec::Property> = HashMap::new();
    for (typ, ref_) in discriminator {
        // {dynamic_type}
        if typ.starts_with('{') {
            continue;
        }
        let mut parameters = HashMap::new();
        let schema = openapi::resolve(components, ref_)?;
        openapi::collect_parameters_into(components, schema, &mut parameters)?;
        parameters.remove("type");
        properties.insert(
            openapi::schema_name(ref_).to_string(),
            spec::Property {
                name: typ.to_string(),
                parameters,
            },
    ...
    let mut types: HashMap<String, spec::Definition> = HashMap::new();
    ...
    while let Some(type_ref) = queue.pop() {
            ...
            && let Some(schema) = schema_ref.as_item()
        {
            ...
            if !parameters.is_empty() {
                ...
                types.insert(type_ref, spec::Definition { parameters });
            }
        }
    }
...
/// Compile a [`Spec`](spec::Spec) into a new [`File`](proto::File).
pub fn compile(package: &str, spec: &spec::Spec, tag_offset: u32) -> Result<proto::File> {
    let mut file = proto::File::new(package);
    compile_into(spec, &mut file, tag_offset)?;
    Ok(file)
}

/// Compile a [`Spec`](spec::Spec) into an existing [`File`](proto::File).
pub fn compile_into(spec: &spec::Spec, file: &mut proto::File, tag_offset: u32) -> Result<()> {
...
mod tests {
    macro_rules! snapshot_tests {
        ($mod:ident, $spec_path:expr, $package:expr, $tag_offset:expr) => {
            mod $mod {
                ...
                fn compile() {
                    let openapi = load_openapi();
                    let spec = crate::extract(&openapi).unwrap();
                    let file = crate::compile($package, &spec, $tag_offset).unwrap();
                    insta::with_settings!({ sort_maps => true }, {
